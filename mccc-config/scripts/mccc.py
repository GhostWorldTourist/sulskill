#!/usr/bin/env python3
"""Read and edit MC Command Center settings, managed as an installable mod.

Edits are written to a managed Vortex staging mod (not the live deployed file),
then repackaged; the user redeploys in Vortex to apply. Without Vortex, use
--direct to write the live file instead.
"""
import argparse
import collections
import datetime
...
LIVE = os.path.join(SIMS, r"Mods\Vortex Mods\mc_settings.cfg")
DRESSER = os.path.join(SIMS, r"Mods\Vortex Mods\mc_dresser.cfg")
...
MANAGED = "ClaudeMCCCSettings"          # the mod this skill owns
...
def build_zip(data, out, name):
    with zipfile.ZipFile(out, 'w', zipfile.ZIP_DEFLATED) as z:
        ...
        z.writestr('README.txt',
                   ...
                   "mccc-config skill. mc_settings.cfg sits at the archive root so\n"
...
def cmd_init(args):
    if not have_vortex():
        sys.exit("Vortex staging not found; use --direct to edit the live file.")
