---
name: mccc-config
description: Read, explain, search, and edit MC Command Center settings for The Sims 4 without using the in-game pie menus. Manages settings as a Vortex-installable mod. Use when the user asks what MCCC settings exist or do, what is possible to configure, or wants to change, compare, back up, or ship MCCC settings (mc_settings.cfg / mc_dresser.cfg).
---
...
This is the opposite of `mc_dresser.cfg`, which *is* Vortex-deployed (from
`wild_guy_Female_Body_Details`) and therefore must be fixed in staging. Don't
generalize one to the other.

### The `--managed` path (optional)

`--managed` switches to a packaged Vortex mod at
`%APPDATA%\Vortex\thesims4\mods\ClaudeMCCCSettings\`, seeded from live on first
use (all 440 settings, so it is a complete template, never a partial overlay),
rebuilding `ClaudeMCCCSettings.zip` on each write.
